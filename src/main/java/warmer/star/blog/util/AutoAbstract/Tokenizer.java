...
public class Tokenizer {
    ...
    public void readStopwords(String stopwordPath) throws IOException
    {
        ...
        Object sws=redisUtil.get("stopword");
        if(sws!=null) {
            stopword = (HashMap<String, Integer>) redisUtil.get("stopword");
        }
        else{
            ClassPathResource classPathResource = new ClassPathResource(stopwordPath);
            if(!classPathResource.exists()){
                System.out.println("stopwords file does not exist!");
                return;
            }
            BufferedReader inBReader=new BufferedReader(new InputStreamReader(classPathResource.getInputStream()));
            String tmpWord;
            int i=0;
            ...
            inBReader.close();
            redisUtil.set("stopword",stopword,3600);
        }
    ...
    public ArrayList<String> tokenizeChnText(String Content, String stopwordPath)
    {
        StringBuffer buffer=new StringBuffer();
        try {
            readStopwords(stopwordPath);
